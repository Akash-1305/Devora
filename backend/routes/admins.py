...
def pending_reports():
    ...
    dep = admin_department(admin)
    q = Report.query.filter_by(status='Completed')
    if dep!= 'DC': 
...
def verify_report(report_id):
    ...
    if admin_department(admin) not in {r.department,'DC'}: return jsonify({'error':'This report belongs to another department.'}), 403
    if r.status != 'Completed': return jsonify({'error':'Only completed work can be verified.'}), 400
    r.status='Admin Verified'; r.verified_at=datetime.utcnow()
    db.session.add(Verification(userid=admin.id, report_id=r.id, location=r.location, status='Admin Verified'))
    if r.flag=='YES':
        r.status='DC Approval Required'
        create_notification(r.userid,r.id,f'Report {r.id} was verified and now requires DC approval.')
    else:
        r.status='Closed'
        create_notification(r.userid,r.id,f'Your JanSeva report {r.id} has been verified and closed.')
    db.session.commit()
...
def reject_report(report_id):
    ...
    if not admin: return jsonify({'error':'Unauthorized action.'}), 401
    if not r: return jsonify({'error':'Invalid report ID.'}), 404
    if admin_department(admin) != r.department: return jsonify({'error':'This report belongs to another department.'}), 403
    if r.status!='Completed': return jsonify({'error':'Only completed work can be rejected.'}), 400
    r.status='Assigned'; r.completed_at=None
    if r.worker: r.worker.active_works += 1
    db.session.commit()
...
def dc_approve(report_id):
    admin=get_admin(); r=Report.query.get(report_id)
    if not admin or admin.designation!='DC': return jsonify({'error':'Only the DC can approve flagged reports.'}), 403
    if not r: return jsonify({'error':'Invalid report ID.'}), 404
    if r.flag!='YES' or r.status!='DC Approval Required': return jsonify({'error':'This report does not require DC approval.'}), 400
    r.status='DC Approved'; r.dc_approved_at=datetime.utcnow(); db.session.flush()
    r.status='Closed'
    create_notification(r.userid,r.id,f'Your flagged JanSeva report {r.id} received DC approval and is now closed.')
    db.session.commit()
