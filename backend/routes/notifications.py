...
@notfication_bp.get('/api/notifications/<userid>')
...
def nearby_solved():
    try:
        lat = float(request.args['lat']); lon = float(request.args['lon']); userid = request.args['userid']
    except Exception:
        return jsonify({'error':'userid, lat and lon are required.'}), 400
    ...
    for r in Report.query.filter_by(status='Closed').order_by(Report.completed_at.desc()).limit(100).all():
        if distance_meters(lat, lon, r.latitude, r.longitude) <= NEARBY_NOTIFICATION_DISTANCE:
            results.append({'report_id':r.id, 'issue_name': r.issue_name, 'location': r.location, 'workerid':r.workerid, 'message':'An issue reported near your current location has been solved. Would you like to provide feedback?'})
