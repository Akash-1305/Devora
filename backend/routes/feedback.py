...
def add_feedback():
    ...
    if not r or r.status != 'Closed':
        return jsonify({'error':'Feedback is allowed only for closed reports.'}), 400
        if not r.workerid: 
            return jsonify({'error':'No worker is associated with this report.'}), 400
        if Feedback.query.filter_by(userid = data.get('userid'), report_id = r.id).first():
            return jsonify({'error':'Feedback already submitted for this report.'}), 409
            try:
