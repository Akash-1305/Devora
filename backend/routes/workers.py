...
@workers_bp.get('/api/workers/<workerid>')
...
def worker_status(workerid):
    ...
    if status not in {'Available','Unavailable'}: return jsonify({'error':'Invalid worker status.'}), 400
...
def worker_works(workerid):
    if not Worker.query.get(workerid): return jsonify({'error':'Worker not found.'}), 404
...
def start_work(report_id):
    ...
    if r.workerid != data.get('workerid'): return jsonify({'error':'Unauthorized action.'}), 403
    if r.status != 'Assigned': return jsonify({'error':'Only assigned work can be started.'}), 400
    r.status = 'In Progress'; r.started_at = datetime.utcnow(); db.session.commit()
    return jsonify({'message':'Work started.','report':serialize_report(r)})

@workers_bp.put('/api/reports/<report_id>/complete')
def complete_work(report_id):
    r = Report.query.get(report_id); data = request.get_json() or {}
    if not r: return jsonify({'error':'Invalid report ID.'}), 404
    if r.workerid != data.get('workerid'): return jsonify({'error':'Unauthorized action.'}), 403
    if r.status != 'In Progress': return jsonify({'error':'Only work in progress can be completed.'}), 400
